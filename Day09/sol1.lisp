    ...
    (with-open-file (file filename :direction :input)
        (let ((initial nil)
            ...
            (loop for line = (read-line file nil) while line do
                ...
                ; pick last values
                (setq buffer (loop for l in buffer collect (car (last l))))

                ; extrapolate the next sequence value from extrapolated differences
                (setq prev 0)
                (setq buffer (loop for n in (reverse buffer) collect (setq prev (+ n prev))))

                ; sum the extrapolated value
                (incf total (+ (car (last initial)) (car (last buffer))))
            )
            total)))

(format t "sample.txt part1: ~a~%" (calculate-sum "sample.txt"))
(format t "input.txt part1: ~a~%" (calculate-sum "input.txt"))